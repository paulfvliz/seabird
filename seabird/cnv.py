...
class CNV:
    ...
    def raw_data(self):
        if ("instrument_type" in self.attrs) and self.attrs[
            "instrument_type"
        ] == "CTD-bottle":
    ...
    def get_intro(self):
        ...
        for k in self.rule["intro"].keys():
            ...
            if pattern.search(self.parsed["intro"]):
                self.attrs[k] = pattern.search(self.parsed["intro"]).groupdict()[
                    "value"
                ]
                self.parsed["intro"] = pattern.sub("", self.parsed["intro"], count=1)
        try:
            self.attrs["instrument_type"] = self.rule["attributes"]["instrument_type"]
        except:
            if "sbe_model" in self.attrs:
                if self.attrs["sbe_model"] in ["9", "17", "19plus", "19plus V2"]:
    ...
    def get_attrs(self):
        ...
        for k in self.rule["descriptors"].keys():
            ...
            if pattern.search(self.parsed["descriptors"]):
                self.attrs[k] = pattern.search(self.parsed["descriptors"]).groupdict()[
                    "value"
                ]
                self.parsed["descriptors"] = pattern.sub(
                    "", self.parsed["descriptors"], count=1
                )
        # ----
    ...
    def prepare_data(self):
        ...
        if ("attributes" in self.rule) and (
            ...
            self.data[0].attrs = {"id": 0, "name": "bottle"}
            self.data[1].attrs = {"id": 1, "name": "date"}
            self.data[2].attrs = {"id": 2, "name": "time"}
    ...
    def load_bindata(self):
        ...
        data = ma.masked_values(output, float(self.attrs["bad_flag"]), atol=1e-30)
        for i in self.ids:
            attrs = self.data[i].attrs
            self.data[i] = data[:, i]
            self.data[i].attrs = attrs
    ...
    def load_bottledata(self):
        ...
        for rec in re.finditer(self.rule["data"], content, re.VERBOSE):
            ...
            for n, v in enumerate(
                re.findall(r"[-|+|\w|\.]+", rec.groupdict()["values"]), start=3
            ):
                v = _convert(v)
            ...
            # Add std values
            for n, v in enumerate(
                re.findall(r"[-|+|\w|\.]+", rec.groupdict()["values_std"]), start=0
            ):
                v = _convert(v)
                if n in data_std:
        ...
        for std_id, values in data_std.items():
            ...
            if "scan_per_bottle" in self.attrs:
                attrs["cell_method"] += (
                    " (previous " + self.attrs["scan_per_bottle"] + " scans)"
                )
            # Add attributes
    ...
    def products(self):
        ...
        if "timeS" not in self.keys():
            if "timeJ" in self.keys():
                ...
                timeS = ma.masked_all(self["timeJ"].shape, self["timeJ"].dtype)
                ...
                try:
                    ...
                    #   for d in self['timeJ'][ind]]
                    timeS[ind] = ma.array([d.days * 86400 + d.seconds - t0 for d in D])
                ...
                timeS = ma.masked_all(self["timeQ"].shape, self["timeQ"].dtype)
                timeS.set_fill_value(float(self.attrs["bad_flag"]))
                ind = np.nonzero(~ma.getmaskarray(self["timeQ"]))[0]
                try:
                    dref = (
                        self.attrs["datetime"] - datetime(2000, 1, 1)
                    ).total_seconds()
                except:
                    dref = self.attrs["datetime"] - datetime(2000, 1, 1)
    ...
    def get_location(self):
        ...
        if ("LATITUDE" in self.attrs) and (
            re.search(self.rule["LATITUDE"], self.attrs["LATITUDE"], re.VERBOSE)
        ):
            lat = re.search(
                self.rule["LATITUDE"], self.attrs["LATITUDE"], re.VERBOSE
            ).groupdict()
        elif ("notes" in self.raw_header().keys()) and re.search(
            self.rule["LATITUDE"], self.raw_header()["notes"], re.VERBOSE
        ):
            lat = re.search(
                self.rule["LATITUDE"], self.raw_header()["notes"], re.VERBOSE
            ).groupdict()
        try:
            ...
            self.attrs["LATITUDE"] = lat_deg + lat_min / 60.0
        ...
        if ("LONGITUDE" in self.attrs) and (
            re.search(self.rule["LONGITUDE"], self.attrs["LONGITUDE"], re.VERBOSE)
        ):
            lon = re.search(
                self.rule["LONGITUDE"], self.attrs["LONGITUDE"], re.VERBOSE
            ).groupdict()
        elif ("notes" in self.raw_header().keys()) and (
            re.search(self.rule["LONGITUDE"], self.raw_header()["notes"], re.VERBOSE)
        ):
            lon = re.search(
                self.rule["LONGITUDE"], self.raw_header()["notes"], re.VERBOSE
            ).groupdict()
        ...
        try:
            ...
            self.attrs["LONGITUDE"] = lon_deg + lon_min / 60.0
            if lon["hemisphere"] in ["W", "w"]:
                self.attrs["LONGITUDE"] = -self.attrs["LONGITUDE"]
        except:
            pass
    ...
    def check_consistency(self):
        ...
        if "nquan" in self.attrs:
            ...
            if nquan != len(self.keys()):
                module_logger.warning("It was supposed to has %s variables." % (nquan))
